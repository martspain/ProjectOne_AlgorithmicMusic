##########################################################################################################################################
# music.py      Version 4.14         07-Oct-2022       Bill Manaris, John-Anthony Thevos, Marge Marshall, Chris Benson, and Kenneth Hanson

###########################################################################
#
# This file is part of Jython Music.
#
# Copyright (C) 2011-2022 Bill Manaris, John-Anthony Thevos, Marge Marshall, Chris Benson, and Kenneth Hanson
#
...
# REVISIONS:
#
# 4.14   07-Oct-2022 (bm, jt)  Added a Play.code() function to implement music-score-driven arbitrary code execution...
#              in other words, a music-driven code sequencer.  Music notes are used as triggers to execute arbitrary Python functions.
#              This is pitch-driven, via a provided dictionary, which associates a particular pitch with a function to execute,
#              when that pitch is triggered.  The pitch is not sounded - only its corresponding function is executed, which 
#              - among other things - could also sound the pitch if it wants to.  Provided functions should accept six parameters,
#              namely pitch, start, duration, velocity, channel, panning, and could do whatever they want with them (including
#              ignoring them).
#
...
#              global jSyn synthesizer.  Now, we create a new jSyn synthesizer inside every new AudioSample.
#              Updated to latest version of jSyn - clicking (which was a big issue) is now gone... YES!!!
...
#OUR_PITCHBEND_NORMAL = 0 
   ...
   def audio(material, listOfAudioSamples, listOfEnvelopes = None, loopAudioSamples = None):
        ...
        # loop through all parts and phrases to get all notes
         ...
         for start, duration, pitch, velocity, channel, instrument, panning in noteList:
            ...
            if duration == 0:   # does this note belong in a chord?
               ...
               if loopAudioSamples:     # did they provide loop flags?
                  Play.audioNote(pitch, start, duration, listOfAudioSamples[channel], velocity, panning, listOfEnvelopes[channel], loopAudioSamples[channel])
               elif listOfEnvelopes:  # if not, did they provide envelopes?
                  Play.audioNote(pitch, start, duration, listOfAudioSamples[channel], velocity, panning, listOfEnvelopes[channel])
               else:                   # no, so just play the audio samples, as is
   ...
   def audioNote(pitch, start, duration, audioSample, velocity = 127, panning = -1, envelope = None, loopAudioSample = False):
      ...
      audioOn  = Timer2(start, Play.audioOn, [pitch, audioSample, velocity, panning, envelope, loopAudioSample], False)
   ...
   def code(material, functions):
      ...
      if type(material) == Score:
         ...
         for start, duration, pitch, velocity, channel, instrument, panning in noteList:
            ...
            # of the chord).
            if duration == 0:   # does this note belong in a chord?
               chordNotes.append([start, duration, pitch, velocity, channel, instrument, panning])  # add it to the list of chord notes
               ...
               if len(functions) > channel:   # is there a function associated with this channel?
                  ...
                  functionTimer = Timer2(start, function, [start, duration, pitch, velocity, channel, instrument, panning], False)
                  functionTimer.start()
               ...
               # first, add this note together with this other chord notes
               chordNotes.append([start, duration, pitch, velocity, channel, instrument, panning])
               
               # now, schedule all notes in the chord list using last note's duration
               for start, ignoreThisDuration, pitch, velocity, channel, instrument, panning in chordNotes:
                  # schedule this note using chord's duration (provided by the last note in the chord)
                  #Play.note(pitch, start, duration, velocity, channel, panning)
...
class AudioSample():
   ...
   def __init__(self, filename, actualPitch=A4, volume=127, voices=16):
      ...
      # stores timers set by envelope (if one of provided at Play time)
      self.envelopeTimers = None 
      ...
      # so, start everything
      self.synth.start()   # start synth engine
...
class Envelope():
   ...
   def performAttackDelaySustain(self, audioSample, volume, voice):
      ...
      if audioSample.envelopeTimers == None:   # is this the first time we see this audio sample?

         audioSample.envelopeTimers = {}                # yes, so initiliaze dictionary of envelope timers
         #print "init envelopeTimers for", audioSample #***
      ...
      for attackTime, attackVolume in zip(self.attackTimes, self.attackVolumes):
         ...
         timer = Timer2(nextTime + someMoreTime, audioSample.setVolume, [volume, attackTime, voice], False)

         #def doIt(volume, attackTime, voice):
         #	print "setting volume now for audioSample - volume, attackTime, voice", audioSample, " - ", volume, attackTime, voice

         #timer = Timer2(nextTime, doIt, [volume, attackTime, voice], False)
         timer.start()
      ...
      someMoreTime = 5   # to give a little extra time for things to happen (just in case) - in milliseconds (avoids clicking...)
      timer = Timer2(nextTime + someMoreTime, audioSample.setVolume, [volume, self.delayTime, voice], False)
      timer.start()
...
def __stopActiveAudioSamples__():
   ...
   for a in __ActiveAudioSamples__:
      ...
      # also, stop each internal synth
      a.synth.stop()   # this is VERY important to do (otherwise it leaves things hanging...)
      ...
      #def deleteAudioSample(audioSample):
      #   del audioSample        # and deallocate the rest

   def deleteSynth():
      del jSynAudioEngine  # deallocate Java jSyn synth

   # schedule deletion after a little time to give everything a chance to shut down properly (and hopefully to avoid clicking)
   delay = delay + 250   # in miliseconds
   deleteTimer = Timer2(delay, deleteSynth, [a], False)
   deleteTimer.start() 
