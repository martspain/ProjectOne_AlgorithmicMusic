...
def getCleanGuitarPart():
   cleanGuitarPart = Part("Clean Guitar", 27, 0)
   cleanPhrase = Phrase()
   cleanSoloPhrase = Phrase()
   cleanIndividualPhrase = Phrase() 
  
   for p in [cleanPhrase, cleanIndividualPhrase]:
      p.setDynamic(F)
      p.setTempo(123)
    
   cleanSoloPhrase.setDynamic(FF)
   cleanSoloPhrase.setTempo(123)
   # Intro
   ...
   cleanPhrase.addNoteList([REST] * 9 + cleanNotes * 3, [WN] * 9 + cleanNotesDurations * 3)
   
   # Añadir el tema al Part
   cleanGuitarPart.addPhrase(cleanPhrase)
   ...
   for pitches, duration in zip(chords_pitches, chords_durations):
       cleanSoloPhrase.addChord(pitches, duration)
   
   cleanGuitarPart.addPhrase(cleanSoloPhrase)
   ...
   cleanIndividualPhrase.addNoteList(bridgeNotes * 4, bridgeNotesDurations * 4)
   cleanGuitarPart.addPhrase(cleanIndividualPhrase)
   
   return cleanGuitarPart
...
def getPianoPart():
   pianoPart = Part("Piano", 0, 0)
   pianoPhrase = Phrase()   
   PianoPitches0   = [D5, B4, F4, D5, E5, E5, C5, G4, E5, D5, C5, D5, B4, G4, D5, E5, E5, B4, G4, E5, D5, C5]
   PianoDurations0 = [QN, QN, QN, EN, EN, EN, QN, QN, EN, EN, EN] * 2
   
   PianoPitches1 = [REST] * 5 + PianoPitches0 * 3
   PianoDurations1 = [WN] * 5 + PianoDurations0 * 3
   
   # create an empty phrase, and construct theme from the above motifs
   
   pianoPhrase.addNoteList(PianoPitches1, PianoDurations1)
...
cleanGuitarPart = getCleanGuitarPart()
pianoPart = getPianoPart()

score.addPart(drumsPart)
score.addPart(bassPart)
score.addPart(overdrivenPart)
score.addPart(cleanGuitarPart)
